...
int     call_stack[STACK_SIZE];         // Call Stack을 저장하는 배열 ex) 2
char    stack_info[STACK_SIZE][20];     // Call Stack 요소에 대한 설명을 저장하는 배열 ex) arg2
...
void print_stack()
{
    ...
    for (int i = SP; i >= 0; i--)
    {
        if (call_stack[i] != -1)
            ...
            printf("%d : %s", i, stack_info[i]);    // call_stack이 -1인 경우. 즉, 메타데이터.
